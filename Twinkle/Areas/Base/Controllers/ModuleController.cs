...
namespace Twinkle.Areas.Base.Controllers
{
    public class ModuleController : BaseController
    {
        ...
        {
            return Paging("SELECT * FROM Sys_Module", "ID", client);
        }

        public JsonResult GetPCode(ClientModel client)
        {
            int? ID = client.GetInt("ID") ?? 0;
            string strSQL = "SELECT 0 value,'系统模块' label union SELECT ID value,cTitle label FROM Sys_Module WHERE ISNULL(cRoute,'')='' AND ID<>@ID";
            return Json(new
            {
                status = 0,
